...
function check_sortorder($rubricid) {
	...
	$criterias = $DB->get_records ( "emarking_rubrics_criteria", array (
			"rubricid" => $rubricid 
	) );
	$sortcount = 1;
...
function ajax_get_criterion($criterionid) {
	global $DB;
	return $DB->get_record ( "emarking_rubrics_criteria", array (
			"id" => $criterionid 
	) );
}
...
function ajax_remove_criterion($criterionid) {
	global $DB;
	return $DB->delete_records ( "emarking_rubrics_criteria", array (
			"id" => $criterionid 
	) );
}
...
function ajax_get_level($levelid) {
	global $DB;
	return $DB->get_record ( "emarking_rubrics_levels", array (
			"id" => $levelid 
	) );
}
...
function ajax_remove_level($levelid) {
	global $DB;
	return $DB->delete_records ( "emarking_rubrics_levels", array (
			"id" => $levelid 
	) );
}
function ajax_get_score($criterionid) {
	global $DB;
	$count = $DB->count_records ( "emarking_rubrics_levels", array (
...
function ajax_get_sort_order($rubricid) {
	global $DB;
	$count = $DB->count_records ( "emarking_rubrics_criteria", array (
			"rubricid" => $rubricid 
	) );
	return $count + 1;
}