...
if ($cmid > 0 && ! $emarking = $DB->get_record('emarking', array(
    'id' => $cm->instance
))) {
    print_error(get_string('invalidid', 'mod_emarking') . " id: $cmid");
}
...
$courseurl = new moodle_url($CFG->dirroot . '/course/view.php', array(
    'id' => $course->id
));
...
if ($cmid > 0) {
    $url = new moodle_url('/mod/emarking/print/newprintorder.php', array(
        'cm' => $cm->id
    ));
    ...
    $cancelurl = new moodle_url('/course/view.php', array(
        'id' => $course->id
    ));
}
...
$mform = new emarking_exam_form(null, array(
    'courseid' => $course->id,
    'options' => $options,
    'examid' => $examid,
    'cmid' => $cmid
...
if ($mform->is_cancelled()) {
    ...
    if ($data = $mform->get_data()) {
        ...
        if ($cmid > 0) {
            $previewurl = new moodle_url('/mod/emarking/print/exams.php', array(
                'id' => $cm->id,
                'examid' => $exam->id
            ));
        } else {
