...
if ($options['help']) {
    $help = "Generates attempts for a specific quiz for all students in the course, and exports a LaTeX
...
// All enroled users in the course
$query = 'SELECT u.*
...
// Run the users query
$users = $DB->get_records_sql($query, array($course->id));
...
/*    if(count($attempts) > 0) {
        echo "User $user->firstname $user->lastname already has attempt for quiz $quizobj->get_quiz_name(). Skipping.\n";
        continue;
    }*/
