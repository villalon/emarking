...
$publishgradesform = ($emarking->type == EMARKING_TYPE_NORMAL || $emarking->type == EMARKING_TYPE_PEER_REVIEW) && has_capability ( "mod/emarking:supervisegrading", $context ) && ! $scan;
...
$sqldraftsorusers = ($emarking->type == EMARKING_TYPE_MARKER_TRAINING) ? "{emarking_submission} AS es
...
function emarking_get_userinfo($draft, $course, $emarking) {
	...
	if ($emarking->type == EMARKING_TYPE_NORMAL || $emarking->type == EMARKING_TYPE_PRINT_SCAN || $emarking->type == EMARKING_TYPE_PEER_REVIEW) {
		$userinfo = $emarking->anonymous < 2 && $USER->id != $draft->id ? get_string ( 'anonymousstudent', 'mod_emarking' ) : $OUTPUT->user_picture ( $draft ) . '&nbsp;<a href="' . $profileurl . '">' . $draft->lastname . ', ' . $draft->firstname . '</a>';
	} else {
...
function emarking_get_finalgrade($d, $usercangrade, $issupervisor, $draft, $rubricscores, $emarking) {
	...
	$thisfinalgrade = '-';
	if ((($usercangrade || $issupervisor) && (($d->status >= EMARKING_STATUS_GRADING && $emarking->type != EMARKING_TYPE_PEER_REVIEW) || ($d->status >= EMARKING_STATUS_GRADING && $draft->id != $USER->id && $emarking->type == EMARKING_TYPE_PEER_REVIEW))) || ($d->status >= EMARKING_STATUS_PUBLISHED && $draft->id == $USER->id)) {
...
function emarking_get_actions($d, $emarking, $context, $draft, $usercangrade, $issupervisor, $publishgradesform, $numcriteria, $scan) {
	...
	if (($usercangrade && $d->status >= EMARKING_STATUS_SUBMITTED && $numcriteria > 0) || $d->status >= EMARKING_STATUS_PUBLISHED || ($emarking->type == EMARKING_TYPE_PRINT_SCAN && $d->status >= EMARKING_STATUS_SUBMITTED)) {
		...
		$actionsarray [] = $OUTPUT->action_link ( $popup_url, $label, new popup_action ( 'click', $popup_url, 'emarking' . $d->id, array (
				'menubar' => 'no',
				'titlebar' => 'no',
				'status' => 'no',
				'toolbar' => 'no',
	...
	if ($emarking->type == EMARKING_TYPE_NORMAL && $d->qc == 0 && (is_siteadmin ( $USER ) || ($issupervisor && $usercangrade)) && $d->status > EMARKING_STATUS_MISSING) {
	...
	$responseurl = new moodle_url ( '/pluginfile.php/' . $context->id . '/mod_emarking/response/' . $draft->id . '/response_' . $emarking->id . '_' . $d->id . '.pdf' );
	
	// Download PDF button
	if ($emarking->type == EMARKING_TYPE_NORMAL && $d->status >= EMARKING_STATUS_PUBLISHED && $d->qc == 0 && ($d->id == $USER->id || is_siteadmin ( $USER ) || $issupervisor)) {
