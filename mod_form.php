...
class mod_emarking_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $availableexams = $DB->get_records("emarking_exams",
                array(
                    "course" => $COURSE->id,
                    "emarking" => 0));
        ...
        if ($parallelcourses = emarking_get_parallel_courses($COURSE)) {
            ...
            if (count($checkboxes) > 0) {
                $mform->addGroup($checkboxes, 'multicourse', get_string('multicourse', 'mod_emarking'),
                        array(
                            '<br/>'), true);
        ...
        if ($this->_features->advancedgrading and ! empty($this->current->_advancedgradingdata ['methods']) and
    ...
    public function data_preprocessing(&$defaultvalues) {
        ...
        if (isset($_COOKIE ["emarking_headerqr"])) {
            if ($_COOKIE ["emarking_headerqr"] == 1) {
                $defaultvalues ["headerqr"] = 1;
            } else {
                $defaultvalues ["headerqr"] = 0;
            }
        }
