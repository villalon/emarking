...
$url = new moodle_url('/mod/emarking/reports/printdetails.php', array('category'=>$categoryid));
...
SELECT c.id,
    ...
    CASE WHEN ro.shortname = 'editingteacher' THEN GROUP_CONCAT(uen.firstname, ' ', uen.lastname SEPARATOR '#') ELSE '' END AS editingteachernames,
    ...
    CASE WHEN ro.shortname = 'teacher' THEN GROUP_CONCAT(uen.firstname, ' ', uen.lastname SEPARATOR '#') ELSE '' END AS teachernames,
    ...
    CASE WHEN ro.shortname = 'student' THEN GROUP_CONCAT(uen.firstname, ' ', uen.lastname SEPARATOR '#') ELSE '' END AS studentnames,
...
INNER JOIN mdl_context AS ctx ON (ctx.contextlevel = 50 AND ctx.instanceid = c.id)
INNER JOIN mdl_enrol AS en ON (en.courseid = c.id)
INNER JOIN mdl_user_enrolments AS ue ON (ue.enrolid = en.id)
INNER JOIN mdl_role_assignments AS ra ON (ra.contextid = ctx.id AND ra.userid = ue.userid)
INNER JOIN mdl_role AS ro ON (ra.roleid = ro.id)
INNER JOIN mdl_user AS uen ON (ue.userid = uen.id)
$filter
...
foreach($stats as $st) {
    ...
    for($i=0;$i<count($teachers); $i++) {
        $teacherurl = new moodle_url("/user/profile.php", array("id"=>$teacherids[$i]));
